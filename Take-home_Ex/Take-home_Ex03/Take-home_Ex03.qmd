...
+----------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| [**fs**](https://github.com/r-lib/fs)                                                                                                        | For retrieve a list of file names in our directory for import into R.                                         |
+----------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
...
+----------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| [**plotly**](https://github.com/plotly/plotly.py)**, [ggiraph](https://davidgohel.github.io/ggiraph/)**                                      | To create interactive plots.                                                                                  |
+----------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| [**ggridges**](https://wilkelab.org/ggridges/)                                                                                               | For creating ridgeline plots; useful for visualising changes in distributions over time/space.                |
+----------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| [**gganimate**](https://gganimate.com/)                                                                                                      | Extends the grammar of graphics to include the description of animation.                                      |
+----------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| [**patchwork**](https://patchwork.data-imaginist.com/)                                                                                       | For plot composition in R.                                                                                    |
+----------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
...
+----------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+----------------------------------+
| 1        | At the [MSS Historical Daily Records](http://www.weather.gov.sg/climate-historical-daily/) page, there are 3 dropdown menus that allow us to select the weather station, month, and year for data extraction. For our study, we will choose *Changi* from the first dropdown menu. | ![](images/ss1.PNG){width="500"} |
+----------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+----------------------------------+
...
::: {.pkbox .pk data-latex="pk"}

**Hi Prof Kam!**

Updates to code used for visualisation:

-   Instead of using the black border to "highlight" the selection, `opts_hover_inv(css = "opacity:0.25;"),` and `opts_hover(css = "stroke-width:0.5;")` are used instead. This ensures the unselected cells are in much lower opacity in contrast to the cell bar/tile of interest.
-   `scale_fill_gradient2` is used to set mid point of the colour gradient scale, here I used median of the mean temperature. 
...
midpt <- median(change_changi$mean_temperature_c[change_changi$month=="Dec"], na.rm = TRUE)
...
-   Dataset-wise, the only change was creating a *desc2* variable to concatenate the month and year without the station. This is used to update the `data_id`.

-   Updates to code used for visualisation:
    -   `facet_wrap()` to split the visualisation by Station.
    -   We also add `scales = "free_y/x"` to `facet_wrap` so that each chart’s y axis is calculated independently.
    -   [`reorder_within()`](https://rdrr.io/cran/tidytext/man/reorder_within.html) of **tidytext** package is used to reorder the boxplots within each facet. Wasn't expecting ordering within a facet to be a problem until now so this function is a lifesaver! 
        -   The function takes minimally 3 arguments: category (aka the variables we are reordering), count variable (the value which we are reordering by), and facet variable.
    -     `scale_y_reordered()` makes sure that the order set in `reorder_within()` is respected.
...
warmdays2 <- dailytemp %>% 
  ...
  summarise(
    ...
    wdays = sum(warmday == 1),
    pct_warmdays = (sum(warmday)/n())*100) %>% 
  mutate(desc2 = paste0(month, " ", year),
         desc = paste0(station, " in ", month, " ", year)) %>% 
  filter(pct_warmdays != 0)
...
bar2 <- ggplot(warmdays2,
              aes(x = reorder(desc2, -pct_warmdays),
                  y = pct_warmdays,
                  fill = month)
              ) +
  geom_bar_interactive(stat = "identity",
  ...
  facet_wrap(~ station, nrow = 2, strip.position="right", scales = "free_x")+ 
  scale_fill_manual(values = c("#a79271", "#4d6575")) +
  scale_y_continuous(labels = ~ paste0(.x, "%"))+  
  scale_x_discrete(labels = scales::label_wrap(6)) +
  labs(x = "",
...
bp2 <-
  ggplot(dailytemp2, 
  aes(x = maximum_temperature_c,
      y = reorder_within(desc2, 
                  by = maximum_temperature_c,
                  within = station,
                  FUN = median),
      fill = month
      )) +
  ...
  scale_x_continuous(labels = ~ paste0(.x, "°"))+  
  scale_y_reordered()+    
  facet_wrap(~ station, nrow = 2, strip.position="right", scales = "free_y")+  
  labs(
      title="Distribution of Max. Temperatures",
      fill = "Month") +
  theme_minimal() +
...
ggiraph::girafe(code = print(bp2 + bar2),
                width_svg = 6,
                height_svg = 6*0.7,
                options = list(opts_tooltip(css = tooltip_css),
                               opts_hover(css="fill: #d1c2de;"),
