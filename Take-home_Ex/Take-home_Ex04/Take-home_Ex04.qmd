...
pacman::p_load(tidyverse, 
               ...
               reactable,
               ranger, vip, rpart, rpart.plot, caret, tidymodels, gbm, 
               performance, visNetwork)
```
...
data <- stu %>% 
  dplyr::select(c("Math",1:23)) #<< input$var, input$mb_target to change according to user selection
```
...
index <- createDataPartition(data[[1]], #<< input$mb_target
...
# Resampling Options
dt_fitcontrol <- trainControl(method = "none", # input$dt_resamplingmethod
...
```{r}
dtmodel_rpart <- rpart(formula = Math ~.,
                       data = traindata,
                       method = "anova",
...
```{r}
rpart_cp <- dtmodel_rpart$cptable[which.min(dtmodel_rpart$cptable[,"xerror"]),"CP"]
```

```{r}

dtmodel_rpart_tuned <- rpart(formula = Math ~.,
                       data = traindata,
                       method = "anova",
                       control = rpart.control(minsplit = 5, maxdepth = 10))

dtmodel_rpart_pruned <- rpart::prune(dtmodel_rpart_tuned, cp = rpart_cp)
```
...
```{r}
plotcp(dtmodel_rpart_pruned)
```

```{r}
visTree(dtmodel_rpart_pruned,
        edgesFontSize = 12,
        nodesFontSize = 14,
        legendWidth = 0.3)
```
...
```{r}
predict_dt_rpartpruned <- bind_cols(
   Actual = testdata$Math, 
   Predicted = predict(dtmodel_rpart, newdata = testdata),
   Residuals = predict(dtmodel_rpart, newdata = testdata) - testdata$Math
)
```

```{r}
dt_predvsactual_rpartpruned <- 
   ggplot(data = predict_dt_rpartpruned,
          aes(x = Actual, y = Predicted)) +
   geom_point(alpha = 0.2, color = "grey40") +
   geom_smooth(method = "loess", formula = "y ~ x", color="#dfb2e9") +
   geom_abline(intercept = 0, slope = 1, linetype = 2, color = "#20948b", size = 0.8) +
...
dt_predvsactual_rpartpruned + dt_residvsactual_rpartpruned + 
  plot_layout(guides = "collect") +
  plot_layout(widths=c(1,1))
```
...
dtmodel$cptable %>%
   data.frame() %>%
   mutate(min_xerror_idx = which.min(dtmodel$cptable[, "xerror"]),
          rownum = row_number(),
          xerror_cap = dtmodel$cptable[min_xerror_idx, "xerror"] + 
             dtmodel$cptable[min_xerror_idx, "xstd"],
...
```{r}
dtmodel$cptable[which.min(initialdtmodel()$cptable[,"xerror"]),"CP"]
...
```{r}
xg_rec <- recipe(Math ~ ., data = traindata)
```
...
```{r}
write.csv(lcalabels, "data/lcalabels.csv", row.names = FALSE)
```
