...
stu_SG_more_long %>%
  ...
  labs(x = "Score", y = NULL, title = "Median score for Math higher than Science and Reading.", 
         substitle="Symmetrical curves appear to resemble Normal distributions across subject.")+
...
pyr_math <- 
  ...
  scale_y_continuous(breaks = -1:1,
                     labels = abs(-1:1)) +
    labs(x = "", y = NULL, title = "Direct relationship between the time spent on math homework and\n math scores among females, whereas an inverse relationship\n exists for males",
...
pyr_science <- 
  ...
  scale_y_continuous(breaks = -1:1,
                     labels = abs(-1:1)) +
    labs(x = "", y = NULL, title = "Male Science scores appear higher across all durations.",
         subtitle = "Time spent on homework: {current_frame} minutes",
         fill = "Gender") +
        coord_flip(clip="off")+
  annotate(
...
## 6.2 Comparing Scores against Student-Teacher Ratio and School Type

```{r}
# Splits School-Type into 2 columns and populate with Math scores
stu_SG_more_wide <- pivot_wider(stu_SG_more, 
                                names_from = SchType, 
                                values_from = Avg_Math) %>% 
  mutate(logstr = log(StuTeacher_ratio),
         public= as.numeric(public),
         privindependent = as.numeric(privindependent))
```

```{r}
str(stu_SG_more_wide)
```

```{r}
ggplot( ) +
  geom_line(data=stu_SG_more, 
            aes(x= log(StuTeacher_ratio), y = Avg_Math, 
                linetype= SchType
                ),
                ) +
  geom_braid(data = stu_SG_more_wide, 
              aes(x=as.numeric(logstr), 
                  ymin= public, 
                  ymax = privindependent, 
                  fill = after_stat(braid)
                 ),
             method = "line",
              na.rm=TRUE,
              alpha=0.6)+
...
median_str <- median(stu_SG_more$StuTeacher_ratio)
sd_math <- sd(stu_SG_more$Avg_Math)

stu_SG_more %>%
  ggplot(aes(x=log(StuTeacher_ratio), Avg_Math, color=SchType))+
  geom_point(alpha=0.8) +
  theme()


ggplot(stu_SG_more, 
         aes(log(StuTeacher_ratio, Avg_Math))) +
  geom_point(
    aes(
      fill = SchType, 
      #size = body_mass_g
  ...
  geom_text(
   ...
   labs(
    x = "log of Student-Teacher Ratio",
    y = "Math Scores",
...
## 6.3 Comparing scores with parents education level

```{r}
stu_SG_more %>% 
  ggplot(mapping = aes(x = x, y = y)) +
    sm_raincloud( position = position_nudge(x = +0.15),
                 show.legend = FALSE,
...
```{r}
ggplot(stu_SG_more, aes(x = Edu_Father, y = Avg_Math)) + 
  ...
  labs(
    title = "Higher Math Scores with Higher Father's Educational Level",
    x = NULL,
    y = "Math"
  ) +
  theme(
