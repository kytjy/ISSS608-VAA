...
Key points:
-   `dmy()` of the **lubridate** package transforms the SentDate to Date data type.
-   `wday()`, also from **lubridate**, returns the day of the week as a decimal number or an ordered factor with `label = TRUE`. The argument `abbr = FALSE` keep the days spells in full, i.e. Monday instead of Mon. 
-   `mutate()` creates a new column in the data.frame i.e. *Weekday* and the output of `wday()` will save in this newly created field.
...
Checking the results of the correction:
```{r}
glimpse(GAStech_edges)
```

*SendDate* is in time format!
...
-   The basic plotting function is `ggraph()`, which takes the data to be used for the graph and the type of layout desired. Both of the arguments for `ggraph()` are built around *igraph*. Therefore, `ggraph()` can accept either an *igraph* object or a *tbl_graph* object.
...
## 6.2 Working with *facet_edges(): change legend position*
...
## 7.2 Visualising network metrics (without computing col above)
